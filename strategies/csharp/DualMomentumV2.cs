...
namespace PiAiTrader.Strategies
{
    ...
    public class DualMomentumV2 : QCAlgorithm
    {
        ...
        public override void Initialize()
        {
            ...
            // This is for testing only — remove or disable before production.
            Schedule.On(
                DateRules.EveryDay(),
                TimeRules.Every(TimeSpan.FromMinutes(1)),
                () =>
                {
                    // Only act during market hours to avoid submitting orders outside
                    // the valid trading window.
                    ...
                    if (File.Exists("/tmp/force_rebalance"))
                    {
                        ...
                        _lastRebalanceMonth = -1; // Reset so monthly schedule still fires next month
                        Rebalance();
                    }
                }
            );
